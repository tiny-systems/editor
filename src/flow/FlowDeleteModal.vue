          ...
          <DialogPanel
            ...
            <div>
              ...
              <div class="mt-3 text-center sm:mt-5">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">Are you sure you want to delete this <span
                    v-if="store.selectedNode">node</span><span v-else-if="store.selectedEdge">edge</span></p>
                  <p class="p-2" v-if="store.selectedNode">{{ store.selectedNode.data.label }}</p>
                  <p class="p-2" v-if="store.selectedEdge">
                        <span>{{ store.selectedEdge.targetNode ? store.selectedEdge.targetNode.data.label : '' }} &rarr; {{
                            store.selectedEdge.sourceNode ? store.selectedEdge.sourceNode.data.label : ''
                          }} {{ store.selectedEdge.sourceHandle.toUpperCase() }}</span>
                  </p>
                </div>
              </div>
            </div>
            <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense text-center">
              ...
              <button @click="$emit('delete')" data-modal-toggle="popup-modal" type="button"
                      class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm items-center px-5 py-2 text-center mr-2">
                Yes, I'm sure
              </button>
            </div>
          </DialogPanel>
